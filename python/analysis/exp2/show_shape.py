...
def plot_shape_all_person(all_plot_datas):
    fig_col = len(all_plot_datas)*3 - 1
    fig_row = 6*2 + 1
    fig, axs = plt.subplots(fig_row, fig_col, figsize=(fig_col/2-1,fig_row/2))
    [axi.set_axis_off() for axi in axs.ravel()]
    for j in range(6):
        axs[j,0].imshow(conv_to_RGB(all_plot_datas[0][j]))
        axs[j+7,0].imshow(conv_to_RGB(all_plot_datas[0][j+6]))
    for i in range(1,len(all_plot_datas)):
        for l in range(3):
            for j in range(6):
                axs[j,i*3+l-1].imshow(conv_to_RGB(all_plot_datas[i][l*12+j]))
                axs[j+7,i*3+l-1].imshow(conv_to_RGB(all_plot_datas[i][l*12+j+6]))
